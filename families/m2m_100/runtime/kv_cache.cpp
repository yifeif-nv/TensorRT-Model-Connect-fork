...
void M2m100KvCache::write_decode_mask(TensorMap& inputs) {
    if (bound_module_ == nullptr)
        throw std::runtime_error("m2m_100 KV cache is not bound to a decoder");
    const int32_t mask_rank = bound_module_->input_rank(names_.attention_mask);
    if (mask_rank != 2)
        throw std::runtime_error("m2m_100 decoder has an invalid attention-mask rank");
...
void M2m100KvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("m2m_100 KV cache has no cache tensor names");
    ...
    if (module.input_rank(names_.cache_k.front()) != 2 ||
        ...
        throw std::runtime_error("m2m_100 decoder does not match its fixed KV-cache contract");
    }
...
void M2m100KvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
