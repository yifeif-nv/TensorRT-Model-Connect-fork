...
// PixArtPipeline - ITrtModule-based PixArt diffusion pipeline.
// PixArt bundles use T5 + DiT + VAE engines.
// Large projection matmuls require the model-owned cuBLAS helper.
// Remaining preprocessing math stays on the host, and ITrtModule owns engine transfers.
...
PixArtPipeline::PixArtPipeline(std::unique_ptr<ITrtModule> text_encoder,
...
void PixArtPipeline::matmul_bias(const float* lhs, const float* rhs, const float* bias,
    ...
    if (pixart_should_use_gpu_matmul(rows, inner, columns)) {
        if (gpu_matmul_ == nullptr ||
            !gpu_matmul_->run(lhs, rhs, bias, output, rows, inner, columns))
            throw std::runtime_error("PixArt required cuBLAS projection failed");
        return;
    }
...
ImageResult PixArtPipeline::generate_image(const std::string& prompt,
