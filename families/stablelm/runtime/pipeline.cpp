...
StablelmTextGenerationPipeline::StablelmTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<StablelmInferenceState> state,
    StablelmTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("StablelmTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("StablelmTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const StablelmTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(stablelm_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(stablelm_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "StablelmTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const StablelmTextGenConfig& cfg,
                                      int32_t sq, StablelmInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("StablelmTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("StablelmTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<StablelmKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: prefill requires StablelmKvCache");
}

} // namespace

void StablelmTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
...
std::string
StablelmTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
StablelmTextGenerationPipeline::TimedGenResult StablelmTextGenerationPipeline::generate_from_ids(
    ...
    const StablelmSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("StablelmTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_stablelm_sampler(params);
...
int32_t StablelmTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const StablelmSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& StablelmTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
