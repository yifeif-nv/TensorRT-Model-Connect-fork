...
#include "families/locateanything/runtime/image_preprocessor.h"
...
LocateAnythingPipeline::LocateAnythingPipeline(
    std::unique_ptr<ITrtModule> text_decoder, std::unique_ptr<ITrtModule> vision_encoder,
...
TextResult LocateAnythingPipeline::generate(const std::string& prompt,
...
bool gather_vl_prefill_kv_pointers(ITrtModule& prefill, const LocateAnythingConfig& config,
...
LocateanythingKvCache* eligible_vl_prefill_cache(ITrtModule* prefill,
...
void add_text_step_embedding_inputs(ITrtModule& decoder, const LocateAnythingConfig& config,
...
LocateAnythingPipeline::resolve_gen_limits(const TextGenerationConfig& cfg) const {
...
LocateAnythingPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    auto local_sampler = create_locateanything_sampler(params);
