...
inline nlohmann::json extract_preprocessor_index(const std::vector<char>& data, const char*& blob,
    ...
    if (data.size() < sizeof(uint32_t))
        throw std::runtime_error("Z-Image preprocessor.weights is too small");
    uint32_t index_size = 0;
    std::memcpy(&index_size, data.data(), sizeof(index_size));
    if (index_size > data.size() - sizeof(index_size))
        throw std::runtime_error("Z-Image preprocessor.weights index exceeds the section");
    ...
    if (!index.is_object())
        throw std::runtime_error("Z-Image preprocessor.weights index must be a JSON object");
...
inline void load_preprocessor_floats(const nlohmann::json& index, const char* blob,
    ...
    if (!entry.is_object())
        throw std::runtime_error("Z-Image preprocessor index entry must be an object: " + key);
    const auto& offset_value = entry.at("offset");
    if (!offset_value.is_number_integer() && !offset_value.is_number_unsigned())
        throw std::runtime_error("Z-Image preprocessor offset must be an integer: " + key);
    const auto signed_offset = offset_value.get<std::int64_t>();
    if (signed_offset < 0)
        throw std::runtime_error("Z-Image preprocessor offset must be nonnegative: " + key);
    const auto offset = static_cast<std::size_t>(signed_offset);
    const auto& shape = entry.at("shape");
    if (!shape.is_array())
        throw std::runtime_error("Z-Image preprocessor shape must be an array: " + key);
    std::size_t count = 1;
    for (const auto& dimension : shape) {
        if (!dimension.is_number_integer() && !dimension.is_number_unsigned())
            throw std::runtime_error("Z-Image preprocessor shape must contain integers: " + key);
        const auto signed_dimension = dimension.get<std::int64_t>();
        if (signed_dimension <= 0)
            throw std::runtime_error("Z-Image preprocessor dimensions must be positive: " + key);
        const auto size = static_cast<std::size_t>(signed_dimension);
        if (count > std::numeric_limits<std::size_t>::max() / size)
            throw std::runtime_error("Z-Image preprocessor shape overflows: " + key);
        count *= size;
    }
    if (count > std::numeric_limits<std::size_t>::max() / sizeof(float))
        throw std::runtime_error("Z-Image preprocessor byte count overflows: " + key);
    const std::size_t byte_count = count * sizeof(float);
    if (offset > blob_size || byte_count > blob_size - offset)
        throw std::runtime_error("Z-Image preprocessor entry exceeds the blob: " + key);
    output.resize(count);
