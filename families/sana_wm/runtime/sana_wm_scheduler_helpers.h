...
inline void fill_dynamic_shift_schedule(FlowMatchEulerPlan& plan, int32_t num_steps,
    ...
    for (int32_t i = 0; i < num_steps; ++i) {
        ...
        plan.sigmas[idx] = static_cast<float>(exp_mu / (exp_mu + (1.0 / raw_sigma_clamped - 1.0)));
        plan.timesteps[idx] = plan.sigmas[idx] * static_cast<float>(N);
    }
}

inline void fill_shifted_linear_schedule(FlowMatchEulerPlan& plan, int32_t num_steps,
                                         const FlowMatchEulerConfig& config, double N) {
    const float shift_f = config.shift;
    const double shift = std::round(static_cast<double>(shift_f) * 1.0e6) / 1.0e6;
    const float raw_sigma_min_f = config.use_zero_sigma_min ? 0.0F : 1.0F / static_cast<float>(N);
    const float shifted_numerator = shift_f * raw_sigma_min_f;
    const volatile float shifted_product = (shift_f - 1.0F) * raw_sigma_min_f;
    const float shifted_denominator = 1.0F + shifted_product;
    const float initial_sigma_min =
        config.use_zero_sigma_min ? 0.0F : shifted_numerator / shifted_denominator;
    const double t_min = static_cast<double>(initial_sigma_min) * N;

    for (int32_t i = 0; i < num_steps; ++i) {
        const double frac =
            static_cast<double>(i) / static_cast<double>(std::max(num_steps - 1, 1));
        const double raw_sigma = (N + frac * (t_min - N)) / N;
        const double sigma = shift * raw_sigma / (1.0 + (shift - 1.0) * raw_sigma);
        const auto idx = static_cast<std::size_t>(i);
        plan.sigmas[idx] = static_cast<float>(sigma);
        plan.timesteps[idx] = plan.sigmas[idx] * static_cast<float>(N);
    }
}
...
inline void apply_terminal_shift(FlowMatchEulerPlan& plan, int32_t num_steps,
    ...
    for (int32_t i = 0; i < num_steps; ++i) {
        ...
        sigma = static_cast<float>(1.0 - ((1.0 - static_cast<double>(sigma)) / scale));
        plan.timesteps[idx] = sigma * static_cast<float>(N);
    }
}
...
struct FlowMatchEulerState {
    ...
    void step(const float* velocity, const float* sample, float* output, std::size_t count,
        ...
        const float sigma_next = sigmas[static_cast<std::size_t>(step_index) + 1];
        const float dt = sigma_next - sigma;
        for (std::size_t i = 0; i < count; ++i) {
            output[i] = sample[i] + dt * velocity[i];
        }
    }
};
