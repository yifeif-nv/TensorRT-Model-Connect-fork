...
"""SANA-WM-owned typed configuration for its local builder components."""
...
class ModelConfig:
    ...
    def from_dir(model_dir: str | Path) -> ModelConfig:
        ...
        family_config = config_from_dir(model_path)
        if family_config is not None:
            return ModelConfig.from_json(json.dumps(family_config))
        raise FileNotFoundError(f"SANA-WM config not found under {model_path}")
