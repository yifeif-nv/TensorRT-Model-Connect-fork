...
PhiMoeTextGenerationPipeline::PhiMoeTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<PhiMoeInferenceState> state,
    ...
    if (prefill_ && !prefill_->ok())
        throw std::runtime_error("PhiMoeTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("PhiMoeTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const PhiMoeTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(phi_moe_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(phi_moe_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "PhiMoeTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const PhiMoeTextGenConfig& cfg,
                                      int32_t sq, PhiMoeInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("PhiMoeTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "PhiMoeTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("PhiMoeTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "PhiMoeTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<PhiMoeKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("PhiMoeTextGenerationPipeline: prefill requires PhiMoeKvCache");
}

} // namespace

void PhiMoeTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "PhiMoeTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "PhiMoeTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
PhiMoeTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
PhiMoeTextGenerationPipeline::TimedGenResult PhiMoeTextGenerationPipeline::generate_from_ids(
    ...
    const PhiMoeSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("PhiMoeTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_phi_moe_sampler(params);
...
ITrtModule& PhiMoeTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
