...
"""StarCoder2-owned tensor-parallel build primitives."""
...
class ParallelConfig:
    ...
    def validate(self) -> None:
        if self.tp_size not in {1, 2, 4, 8}:
            raise ValueError("StarCoder2 tensor_parallel_size must be one of 1, 2, 4, 8")
        if self.rank < -1 or self.rank >= self.tp_size:
            raise ValueError("StarCoder2 tensor-parallel rank is outside the requested world")
...
def shard_standard_decoder_weights(
    ...
    if not parallel.enabled:
        return weights
    if parallel.rank < 0:
        raise ValueError("StarCoder2 tensor-parallel build requires a concrete rank")
    if model_config.num_attention_heads % parallel.tp_size:
        raise ValueError(
            "StarCoder2 num_attention_heads must be divisible by tensor_parallel_size"
        )
    if model_config.num_key_value_heads % parallel.tp_size:
        raise ValueError(
            "StarCoder2 num_key_value_heads must be divisible by tensor_parallel_size"
        )
    mlp_size = int(weights.get("_mlp_size", model_config.intermediate_size))
    if mlp_size % parallel.tp_size:
        raise ValueError(
            "StarCoder2 intermediate size must be divisible by tensor_parallel_size"
        )
...
def add_all_reduce_sum(network, tensor, tp_size: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT failed to create StarCoder2 ALL_REDUCE")
