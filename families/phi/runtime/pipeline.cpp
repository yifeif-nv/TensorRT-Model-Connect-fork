...
PhiTextGenerationPipeline::PhiTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                     std::unique_ptr<PhiInferenceState> state,
                                                     PhiTextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("PhiTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("PhiTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
    ...
    if (cfg.use_chat_template) {
        if (config.chat_template_format.empty())
            throw std::runtime_error(
                "PhiTextGenerationPipeline: checkpoint has no supported chat template");
        effective =
...
void require_prefill_kv_pointers(ITrtModule& prefill, const PhiTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(phi_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(phi_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "PhiTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const PhiTextGenConfig& cfg, int32_t sq,
                                      PhiInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("PhiTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("PhiTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("PhiTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("PhiTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<PhiKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("PhiTextGenerationPipeline: prefill requires PhiKvCache");
}

} // namespace

void PhiTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error("PhiTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "PhiTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
PhiTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
PhiTextGenerationPipeline::TimedGenResult PhiTextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("PhiTextGenerationPipeline: unsupported generation mode '" + mode +
                                 "'");
    auto active_sampler = create_phi_sampler(params);
...
PhiTextGenerationPipeline::run_decode_loop(PhiISampler* sampler, const PhiSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const PhiSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& PhiTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
