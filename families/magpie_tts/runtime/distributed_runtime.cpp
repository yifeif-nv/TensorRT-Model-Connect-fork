...
int require_env_int(const char* name) {
    ...
    if (raw == nullptr || *raw == '\0')
        throw std::runtime_error(std::string("magpie_tts tensor parallel requires ") + name);
    char* end = nullptr;
    long value = std::strtol(raw, &end, 10);
    if (end == raw || *end != '\0')
        throw std::runtime_error(std::string("magpie_tts invalid integer in ") + name);
