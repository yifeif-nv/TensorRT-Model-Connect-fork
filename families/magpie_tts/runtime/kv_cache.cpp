...
void MagpieKvCache::write_decode_mask(TensorMap& inputs) {
    if (bound_module_ == nullptr)
        throw std::runtime_error("magpie_tts KV cache is not bound to a decoder");
    const int32_t mask_rank = bound_module_->input_rank(names_.attention_mask);
    if (mask_rank != 2 && mask_rank != 3)
        throw std::runtime_error("magpie_tts decoder has an invalid attention-mask rank");
    ...
    Tensor mask_t;
    mask_t.data = mask_buf_.data();
    mask_t.shape = mask_rank == 3 ? std::vector<int64_t>{1, 1, static_cast<int64_t>(mask_width)}
                                  : std::vector<int64_t>{1, static_cast<int64_t>(mask_width)};
    mask_t.dtype = DType::kFloat32;
    inputs[names_.attention_mask] = mask_t;
}
...
void MagpieKvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("magpie_tts KV cache has no cache tensor names");
    const auto cache_shape = module.tensor_shape(names_.cache_k.front());
    const std::vector<int64_t> expected_cache_shape{max_length_, kv_dim_};
    if (module.input_rank(names_.cache_k.front()) != 2 ||
        module.input_is_dynamic(names_.cache_k.front()) || cache_shape != expected_cache_shape) {
        throw std::runtime_error("magpie_tts decoder does not match its fixed KV-cache contract");
    }
    const int32_t mask_rank = module.input_rank(names_.attention_mask);
    if (mask_rank != 2 && mask_rank != 3)
        throw std::runtime_error("magpie_tts decoder has an invalid attention-mask rank");
...
void MagpieKvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
