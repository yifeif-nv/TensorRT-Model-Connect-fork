...
"""MagpieTTS tensor-parallel build primitives."""
...
class ParallelConfig:
    ...
    def validate(self) -> None:
        if self.tp_size not in {1, 2, 4, 8}:
            raise ValueError("MagpieTTS tensor_parallel_size must be one of 1, 2, 4, 8")
        if self.rank < -1 or self.rank >= self.tp_size:
            raise ValueError("MagpieTTS rank is outside the requested world")
...
def add_all_reduce_sum(network, tensor, tp_size: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT failed to create MagpieTTS ALL_REDUCE")
