...
MistralTextGenerationPipeline::MistralTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<MistralInferenceState> state,
    MistralTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("MistralTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("MistralTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const MistralTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(mistral_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(mistral_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "MistralTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const MistralTextGenConfig& cfg,
                                      int32_t sq, MistralInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("MistralTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "MistralTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("MistralTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "MistralTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<MistralKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("MistralTextGenerationPipeline: prefill requires MistralKvCache");
}
...
std::string
MistralTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
MistralTextGenerationPipeline::TimedGenResult MistralTextGenerationPipeline::generate_from_ids(
    ...
    const MistralSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("MistralTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_mistral_sampler(params);
...
int32_t MistralTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const MistralSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& MistralTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
