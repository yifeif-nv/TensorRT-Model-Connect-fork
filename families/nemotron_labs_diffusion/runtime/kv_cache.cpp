...
void NemotronLabsDiffusionKvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("nemotron_labs_diffusion KV cache has no cache tensor names");
    if (!module.has_input(names_.attention_mask) ||
        ...
        throw std::runtime_error(
            "nemotron_labs_diffusion decoder has an invalid attention-mask contract");
    }
    ...
    for (int32_t i = 0; i < num_layers_; ++i) {
        ...
        if (!module.has_input(names_.cache_k[layer]) || !module.has_input(names_.cache_v[layer]) ||
            ...
            throw std::runtime_error(
                "nemotron_labs_diffusion decoder does not match its fixed KV contract");
        }
...
void NemotronLabsDiffusionKvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
