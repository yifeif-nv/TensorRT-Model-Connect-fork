...
def _native(
    ...
    for path in frame_paths:
        arguments.extend(("--frame", str(path)))
    arguments.extend(("--prompt", _case_text(case)))
    payload = _run_json(binary, runtime_root, bundle, manifest, case, "video-segment", *arguments)
    payload["input_frames"] = [str(path) for path in frame_paths]
    payload["masks"] = [frame["masks"] for frame in payload["frames"]]
    return payload
