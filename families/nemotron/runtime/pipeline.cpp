...
NemotronTextGenerationPipeline::NemotronTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<NemotronInferenceState> state,
    NemotronTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("NemotronTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("NemotronTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const NemotronTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(nemotron_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(nemotron_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "NemotronTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const NemotronTextGenConfig& cfg,
                                      int32_t sq, NemotronInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("NemotronTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "NemotronTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("NemotronTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "NemotronTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<NemotronKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error(
            "NemotronTextGenerationPipeline: prefill requires NemotronKvCache");
}

} // namespace

void NemotronTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "NemotronTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab)
        throw std::runtime_error(
            "NemotronTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
NemotronTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
NemotronTextGenerationPipeline::TimedGenResult NemotronTextGenerationPipeline::generate_from_ids(
    ...
    const NemotronSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("NemotronTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_nemotron_sampler(params);
...
int32_t NemotronTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const NemotronSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& NemotronTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
