...
DeepseekV2TextGenerationPipeline::DeepseekV2TextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<DeepseekV2InferenceState> state,
    DeepseekV2TextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
...
void require_prefill_kv_pointers(ITrtModule& prefill, const DeepseekV2TextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(deepseek_v2_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(deepseek_v2_expand_layer_name(cfg.present_v_pattern, layer));
...
void require_batched_prefill_contract(ITrtModule& prefill, const DeepseekV2TextGenConfig& cfg,
                                      int32_t sq, DeepseekV2InferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<DeepseekV2KvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: prefill requires DeepseekV2KvCache");
}

} // namespace

void DeepseekV2TextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
DeepseekV2TextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
DeepseekV2TextGenerationPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    if (mode != "ar")
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_deepseek_v2_sampler(params);
...
int32_t DeepseekV2TextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const DeepseekV2SampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& DeepseekV2TextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
