...
def _native(
    ...
    payload["text"] = " ".join(
        (
            ...
            if event.get("kind") == "agent_text" and event.get("text")
        )
    )
    return payload
...
def _assert_parity(actual, expected, manifest: dict, case: dict, thresholds: dict) -> None:
    ...
    assert float(np.sqrt(np.mean(actual_samples**2))) >= float(thresholds["audio_min_rms"])
    assert float(np.max(np.abs(actual_samples))) >= float(thresholds["audio_min_peak"])
