...
def add_layer_norm_no_affine(
    ...
    with all-ones gamma and all-zeros beta. This helper serves call sites that
    only thread epsilon as an ITensor.
    """
    need_cast = dtype != np.float32
    output_dtype = inp.dtype
    if need_cast:
...
def add_attention_core(
    ...
    decomposable=True ensures a correct primitive decomposition otherwise.
