...
std::int32_t require_rank(std::int32_t tp_size) {
    ...
    if (text == nullptr || *text == '\0')
        throw std::runtime_error("DeepSeek-OCR TP runtime requires RANK");
    char* end = nullptr;
...
DeepseekOcrKvCacheNames build_kv_names(const nlohmann::json& config, std::int32_t num_layers) {
    ...
    for (std::int32_t layer = 0; layer < num_layers; ++layer) {
        names.cache_k.push_back(deepseek_ocr_expand_layer_name(cache_k, layer));
        names.cache_v.push_back(deepseek_ocr_expand_layer_name(cache_v, layer));
        names.present_k.push_back(deepseek_ocr_expand_layer_name(present_k, layer));
        names.present_v.push_back(deepseek_ocr_expand_layer_name(present_v, layer));
    }
    return names;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto tp_size = deepseek_ocr_factory::require_positive_int(config, "tensor_parallel_size");
    ...
    const auto num_layers = deepseek_ocr_factory::require_positive_int(config, "num_layers");
    const auto max_cache_length =
        deepseek_ocr_factory::require_positive_int(config, "max_cache_length");
    auto kv_names = deepseek_ocr_factory::build_kv_names(config, num_layers);
    ...
    model.vocab_size = deepseek_ocr_factory::require_positive_int(config, "vocab_size");
    model.id_bos = deepseek_ocr_factory::require_int(config, "id_bos");
    model.id_eos = deepseek_ocr_factory::require_int(config, "id_eos");
    model.image_token_id = deepseek_ocr_factory::require_int(config, "image_token_id");
    model.vision_output_dim =
        deepseek_ocr_factory::require_positive_int(config, "vision_output_dim");
    model.has_position_input = decode.module->has_input("position_id");
    model.num_layers = num_layers;
    model.prefill_max_length =
        deepseek_ocr_factory::require_positive_int(config, "prefill_max_length");
    const auto& io = config.at("io_map");
