...
std::int32_t require_rank(std::int32_t tensor_parallel_size) {
    ...
    if (text == nullptr || *text == '\0')
        throw std::runtime_error("XLNet TP runtime requires OMPI_COMM_WORLD_RANK");
    char* end = nullptr;
    const long rank = std::strtol(text, &end, 10);
    if (*end != '\0' || rank < 0 || rank >= tensor_parallel_size)
        throw std::runtime_error("XLNet RANK is outside tensor_parallel_size");
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto rank = trtmc::xlnet_factory::require_rank(tp_size);
