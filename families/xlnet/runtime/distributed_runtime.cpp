...
int require_env_int(const char* name) {
    ...
    if (raw == nullptr || *raw == '\0')
        throw std::runtime_error(std::string("xlnet tensor parallel requires ") + name);
    char* end = nullptr;
    long value = std::strtol(raw, &end, 10);
    if (end == raw || *end != '\0')
        throw std::runtime_error(std::string("xlnet invalid integer in ") + name);
