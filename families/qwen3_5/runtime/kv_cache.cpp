...
#include "families/qwen3_5/runtime/kv_cache.h"
...
void Qwen35KvCache::write_decode_mask(TensorMap& inputs) {
    if (bound_module_ == nullptr)
        throw std::runtime_error("qwen3_5 KV cache is not bound to a decoder");
    const int32_t mask_rank = bound_module_->input_rank(names_.attention_mask);
    if (mask_rank != 2)
        throw std::runtime_error("qwen3_5 decoder has an invalid attention-mask rank");
...
void Qwen35KvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("qwen3_5 KV cache has no cache tensor names");
    ...
    if (module.input_rank(names_.cache_k.front()) != 2 ||
        ...
        throw std::runtime_error("qwen3_5 decoder does not match its fixed KV-cache contract");
    }
...
void Qwen35KvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
