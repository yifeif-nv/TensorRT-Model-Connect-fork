...
void BartKvCache::write_decode_mask(TensorMap& inputs) {
    if (bound_module_ == nullptr)
        throw std::runtime_error("bart KV cache is not bound to a decoder");
    const int32_t mask_rank = bound_module_->input_rank(names_.attention_mask);
    if (mask_rank != 1)
        throw std::runtime_error("bart decoder has an invalid attention-mask rank");
...
void BartKvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("bart KV cache has no cache tensor names");
    ...
    if (module.input_rank(names_.cache_k.front()) != 2 ||
        ...
        throw std::runtime_error("bart decoder does not match its fixed KV-cache contract");
    }
...
void BartKvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
