...
GlmTextGenerationPipeline::GlmTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                     std::unique_ptr<GlmInferenceState> state,
                                                     GlmTextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("GlmTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("GlmTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const GlmTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(glm_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(glm_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "GlmTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const GlmTextGenConfig& cfg, int32_t sq,
                                      GlmInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("GlmTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("GlmTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("GlmTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("GlmTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<GlmKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GlmTextGenerationPipeline: prefill requires GlmKvCache");
}

} // namespace

void GlmTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error("GlmTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "GlmTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
GlmTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
GlmTextGenerationPipeline::TimedGenResult GlmTextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("GlmTextGenerationPipeline: unsupported generation mode '" + mode +
                                 "'");
    auto active_sampler = create_glm_sampler(params);
...
GlmTextGenerationPipeline::run_decode_loop(GlmISampler* sampler, const GlmSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const GlmSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& GlmTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
