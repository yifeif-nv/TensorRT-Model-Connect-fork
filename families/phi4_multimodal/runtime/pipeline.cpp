...
TextResult Phi4MultimodalPipeline::generate(const std::string& prompt,
...
bool gather_vl_prefill_kv_pointers(ITrtModule& prefill, const Phi4MultimodalConfig& config,
...
Phi4MultimodalKvCache* eligible_vl_prefill_cache(ITrtModule* prefill,
...
void add_text_step_embedding_inputs(ITrtModule& decoder, const Phi4MultimodalConfig& config,
...
Phi4MultimodalPipeline::resolve_gen_limits(const TextGenerationConfig& cfg) const {
...
Phi4MultimodalPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    auto local_sampler = create_phi4_multimodal_sampler(params);
