...
def build_standard_decoder_engine(
    ...
    *,
    ...
    # alternate reaches this family with role="decode". Keep that engine
    # dual-profile when vision embeddings are enabled.
    vl_decode_engine = embed_input and decoder_engine_role == "decode"
    if not _dual_profile_disabled_for and (
        decoder_engine_role in ("dual_profile", "prefill") or vl_decode_engine
    ):
