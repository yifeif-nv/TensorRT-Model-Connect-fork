...
#include "families/internvl/runtime/image_preprocessor.h"
...
void validate_pipeline_components(const ITrtModule* text_decoder,
                                  const InternvlInferenceState* state, const ITrtModule* prefill) {
...
InternVlPipeline::InternVlPipeline(std::unique_ptr<ITrtModule> text_decoder,
                                   ...
                                   std::string model_id_str, std::unique_ptr<ITrtModule> prefill)
...
TextResult InternVlPipeline::generate(const std::string& prompt, const TextGenerationConfig& cfg) {
    if (!tokenizer_)
...
bool gather_vl_prefill_kv_pointers(ITrtModule& prefill, const InternVlConfig& config,
...
InternvlKvCache* eligible_vl_prefill_cache(ITrtModule* prefill, InternvlInferenceState* state,
...
void add_text_step_embedding_inputs(ITrtModule& decoder, const InternVlConfig& config,
...
InternVlPipeline::resolve_gen_limits(const TextGenerationConfig& cfg) const {
...
InternVlPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                               const TextGenerationConfig& cfg) {
...
std::vector<int32_t> InternVlPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    auto local_sampler = create_internvl_sampler(params);
