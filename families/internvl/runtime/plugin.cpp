...
std::int32_t require_rank(std::int32_t tp_size) {
    ...
    if (text == nullptr || *text == '\0')
        throw std::runtime_error("InternVL TP runtime requires OMPI_COMM_WORLD_RANK");
    char* end = nullptr;
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto rank = internvl_factory::require_rank(tp_size);
    const std::string decode_section =
        tp_size == 1 ? "engine.plan" : "engine.rank" + std::to_string(rank) + ".plan";
    const auto& decode_plan =
        internvl_factory::require_section(context.reader, decode_section.c_str());
    const auto& prefill_plan = internvl_factory::require_section(context.reader, "prefill.plan");
