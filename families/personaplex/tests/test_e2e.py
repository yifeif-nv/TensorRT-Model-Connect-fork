...
def _assert_parity(actual, expected, manifest: dict, case: dict, thresholds: dict) -> None:
    ...
    actual_tokens = np.asarray(actual["speech_tokens"], dtype=np.int32)
    expected_tokens = np.asarray(expected["speech_tokens"], dtype=np.int32)
    frames = min(actual_tokens.shape[0], expected_tokens.shape[0])
    assert frames > 0
    actual_tokens = actual_tokens[:frames]
    expected_tokens = expected_tokens[:frames]
    assert actual_tokens.shape == expected_tokens.shape
    token_agreement = float(np.mean(actual_tokens == expected_tokens))
    depth_agreement = float(np.mean(actual_tokens[:, 1:] == expected_tokens[:, 1:]))
    frame_agreement = float(np.mean(np.all(actual_tokens == expected_tokens, axis=1)))
    assert token_agreement >= float(thresholds["audio_token_match_rate"])
    ...
    assert frame_agreement >= float(thresholds["frame_exact_match_rate"])
    assert frame_agreement >= float(thresholds["speech_min_frame_exact"])
    samples, _ = sf.read(actual["audio"], dtype="float32")
    assert float(np.sqrt(np.mean(np.asarray(samples) ** 2))) >= float(thresholds["speech_min_rms"])
    return
