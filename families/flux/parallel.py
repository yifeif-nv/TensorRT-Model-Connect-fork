...
"""FLUX tensor- and context-parallel build primitives."""
...
class ParallelConfig:
    ...
    def mode(self) -> str:
        ...
        return "tensor_parallel" if self.enabled else "single"
    ...
    def validate(self) -> None:
        if self.tp_size not in {1, 2, 4, 8}:
            raise ValueError("FLUX tensor_parallel_size must be one of 1, 2, 4, 8")
        if self.cp_size not in {1, 2, 4, 8}:
            raise ValueError("FLUX context_parallel_size must be one of 1, 2, 4, 8")
        if self.enabled and self.cp_enabled:
            raise ValueError("FLUX tensor and context parallelism cannot be combined")
        if self.rank < -1 or self.rank >= self.world_size:
            raise ValueError("FLUX rank is outside the requested world")
...
def add_all_reduce_sum(network, tensor, tp_size: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT failed to create FLUX ALL_REDUCE")
...
def add_dynamic_batch_profile(
    ...
    *,
    ...
    if max_batch < 1 or not 1 <= opt_batch <= max_batch:
        raise ValueError("FLUX dynamic batch profile is invalid")
