...
int require_env_int(const char* name) {
    ...
    if (raw == nullptr || *raw == '\0')
        throw std::runtime_error(std::string("FLUX distributed runtime requires ") + name);
    char* end = nullptr;
    const long value = std::strtol(raw, &end, 10);
    if (end == raw || *end != '\0')
        throw std::runtime_error(std::string("FLUX distributed runtime has invalid integer in ") +
                                 name);
...
std::filesystem::path rendezvous_path() {
    ...
    if (path == nullptr || *path == '\0')
        throw std::runtime_error("FLUX distributed runtime requires TRTMC_NCCL_RENDEZVOUS");
    return path;
}
...
class NcclRuntime {
  public:
    NcclRuntime() {
        ...
        if (handle_ == nullptr) {
            throw std::runtime_error(std::string("Failed to load NCCL for FLUX runtime: ") +
                                     dlerror());
        }
...
void bind_cuda_device() {
    ...
    if (count_status != cudaSuccess) {
        throw std::runtime_error(std::string("cudaGetDeviceCount failed for FLUX runtime: ") +
                                 cudaGetErrorString(count_status));
    }
    if (local_rank < 0 || local_rank >= count)
        throw std::runtime_error("FLUX local rank is outside the visible CUDA device range");
    const auto status = cudaSetDevice(local_rank);
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("cudaSetDevice failed for FLUX local rank: ") +
...
DistributedGroup initialize_group(int size) {
    ...
    if (size <= 1)
        return group;
    group.world_size = require_env_int("OMPI_COMM_WORLD_SIZE");
    group.rank = require_env_int("OMPI_COMM_WORLD_RANK");
    if (group.world_size != size)
        throw std::runtime_error("FLUX distributed runtime requires launcher world size to equal "
                                 "parallel_size");
    if (group.rank < 0 || group.rank >= size)
        throw std::runtime_error("FLUX rank is outside [0, parallel_size)");
    ...
    runtime->init(size, group.rank, id);
