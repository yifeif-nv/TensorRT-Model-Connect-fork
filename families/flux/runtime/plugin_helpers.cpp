...
std::shared_ptr<ITokenizer> create_tokenizer(const std::vector<char>& data, bool add_special,
                                             std::int32_t text_encoder_count) {
    std::unique_ptr<ITokenizer> tokenizer;
    if (text_encoder_count == 1)
        tokenizer = CreateBpeTokenizer(data.data(), data.size(), add_special);
    else if (text_encoder_count == 2)
        tokenizer = CreateUnigramTokenizer(data.data(), data.size(), add_special);
    else
        throw std::runtime_error("FLUX requires one or two text encoders");
    if (!tokenizer)
        throw std::runtime_error("tokenizer.json does not match the FLUX model variant");
...
std::shared_ptr<ITokenizer> create_tokenizer_from_bundle(const BundleReader& bundle) {
    ...
    auto tokenizer = create_tokenizer(require_section(bundle, "tokenizer.json"),
                                      ...
                                      runtime.at("num_text_encoders").get<std::int32_t>());
...
std::unique_ptr<ITokenizer> create_clip_tokenizer_from_bundle(const BundleReader& bundle) {
    const auto& data = require_section(bundle, "clip_tokenizer.json");
    auto tokenizer = CreateBpeTokenizer(data.data(), data.size(), true);
    if (!tokenizer)
        throw std::runtime_error("clip_tokenizer.json is not BPE");
    return tokenizer;
}
