...
GptOssTextGenerationPipeline::GptOssTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<GptOssInferenceState> state,
    ...
    if (prefill_ && !prefill_->ok())
        throw std::runtime_error("GptOssTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("GptOssTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const GptOssTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(gpt_oss_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(gpt_oss_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "GptOssTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const GptOssTextGenConfig& cfg,
                                      int32_t sq, GptOssInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("GptOssTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("GptOssTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<GptOssKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GptOssTextGenerationPipeline: prefill requires GptOssKvCache");
}

} // namespace

void GptOssTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
GptOssTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
GptOssTextGenerationPipeline::TimedGenResult GptOssTextGenerationPipeline::generate_from_ids(
    ...
    const GptOssSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("GptOssTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_gpt_oss_sampler(params);
...
int32_t GptOssTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const GptOssSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& GptOssTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
