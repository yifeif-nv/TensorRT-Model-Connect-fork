...
"""ModernBERT-owned tensor-parallel build primitives."""
...
def shard_standard_decoder_weights(
    ...
    if model_config.num_attention_heads % parallel.tp_size:
        raise ValueError(
            "ModernBERT num_attention_heads must be divisible by tensor_parallel_size"
        )
    if model_config.num_key_value_heads % parallel.tp_size:
        raise ValueError(
            "ModernBERT num_key_value_heads must be divisible by tensor_parallel_size"
        )
    mlp_size = int(weights.get("_mlp_size", model_config.intermediate_size))
    if mlp_size % parallel.tp_size:
        raise ValueError(
            "ModernBERT intermediate size must be divisible by tensor_parallel_size"
        )
