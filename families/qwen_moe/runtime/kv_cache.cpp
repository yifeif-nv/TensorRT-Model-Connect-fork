...
void QwenMoeKvCache::bind_to(ITrtModule& module) {
    if (names_.cache_k.empty())
        throw std::runtime_error("qwen_moe KV cache has no cache tensor names");
    if (!module.has_input(names_.attention_mask) ||
        module.tensor_shape(names_.attention_mask) != std::vector<int64_t>{1, max_length_ + 1}) {
        throw std::runtime_error("qwen_moe decoder has an invalid attention-mask contract");
    }
    ...
    for (int32_t i = 0; i < num_layers_; ++i) {
        ...
        if (!module.has_input(names_.cache_k[layer]) || !module.has_input(names_.cache_v[layer]) ||
            ...
            throw std::runtime_error("qwen_moe decoder does not match its fixed KV contract");
        }
...
void QwenMoeKvCache::bind_cache_inputs(ITrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
