...
GraniteTextGenerationPipeline::GraniteTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<GraniteInferenceState> state,
    GraniteTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("GraniteTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("GraniteTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const GraniteTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(granite_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(granite_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "GraniteTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const GraniteTextGenConfig& cfg,
                                      int32_t sq, GraniteInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("GraniteTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "GraniteTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("GraniteTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "GraniteTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<GraniteKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GraniteTextGenerationPipeline: prefill requires GraniteKvCache");
}
...
std::string
GraniteTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
GraniteTextGenerationPipeline::TimedGenResult GraniteTextGenerationPipeline::generate_from_ids(
    ...
    const GraniteSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("GraniteTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_granite_sampler(params);
...
int32_t GraniteTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const GraniteSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& GraniteTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
