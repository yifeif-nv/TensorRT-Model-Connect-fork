...
InternlmTextGenerationPipeline::InternlmTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<InternlmInferenceState> state,
    InternlmTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("InternlmTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("InternlmTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const InternlmTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(internlm_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(internlm_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "InternlmTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const InternlmTextGenConfig& cfg,
                                      int32_t sq, InternlmInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("InternlmTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "InternlmTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("InternlmTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "InternlmTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<InternlmKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error(
            "InternlmTextGenerationPipeline: prefill requires InternlmKvCache");
}
...
std::string
InternlmTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
InternlmTextGenerationPipeline::TimedGenResult InternlmTextGenerationPipeline::generate_from_ids(
    ...
    const InternlmSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("InternlmTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_internlm_sampler(params);
...
int32_t InternlmTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const InternlmSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& InternlmTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
