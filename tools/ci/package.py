...
class InstalledWheelValidator:
    ...
    def __init__(self, repository: Path, _tensorrt_version: str | None = None):
...
for family in sys.argv[1:]:
    importlib.import_module(f"families.{family}.model")
print(json.dumps({
...
"""
        ...
        completed = subprocess.run(
            [sys.executable, "-c", script, *family_ids(self.repository)],
            check=True,
            capture_output=True,
            text=True,
            cwd=Path("/tmp"),
