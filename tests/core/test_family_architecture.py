...
def test_shared_test_tree_contains_only_core_and_active_ci_contracts() -> None:
    allowed_files = {
        "tests/__init__.py",
        "tests/core/test_build.py",
        "tests/core/test_build_cli.py",
        ...
        "tests/cpp/core/test_bundle_format_v1.cpp",
        "tests/cpp/core/test_byok_shape_spec.cpp",
        "tests/cpp/core/test_cli.cpp",
        "tests/cpp/core/test_dataset_answer.cpp",
        "tests/cpp/core/test_family_loader.cpp",
        "tests/cpp/core/test_task_api.cpp",
        "tests/tools/__init__.py",
        "tests/tools/test_community_ci.py",
        "tests/tools/test_cosmos3_dual_spark_example.py",
        ...
        "tests/tools/test_voicechat_full_duplex_example.py",
    }
    actual = {
    ...
    assert sorted(actual - allowed_files) == []
    assert sorted(allowed_files - actual) == []
...
def test_shared_python_and_native_trees_are_closed_minimal_sets() -> None:
    expected_python = {
        ...
        "python/tensorrt_model_connect/build.py",
    ...
    expected_native = {
        ...
        "src/runtime/backend/trt_logger.h",
    ...
    python_files = files("python/tensorrt_model_connect")
    benchmark_files = {
        path for path in python_files if path.startswith("python/tensorrt_model_connect/benchmark/")
    }
    expected_benchmark = {
        "python/tensorrt_model_connect/benchmark/__init__.py",
        "python/tensorrt_model_connect/benchmark/__main__.py",
        "python/tensorrt_model_connect/benchmark/builder.py",
        "python/tensorrt_model_connect/benchmark/catalog.py",
        "python/tensorrt_model_connect/benchmark/cli.py",
        "python/tensorrt_model_connect/benchmark/metrics.py",
        "python/tensorrt_model_connect/benchmark/operations.py",
        "python/tensorrt_model_connect/benchmark/report.py",
        "python/tensorrt_model_connect/benchmark/service.py",
        "python/tensorrt_model_connect/benchmark/task_adapters.py",
        "python/tensorrt_model_connect/benchmark/telemetry.py",
        "python/tensorrt_model_connect/benchmark/tests/test_benchmark.py",
        "python/tensorrt_model_connect/benchmark/tests/test_perf_matrix.py",
        "python/tensorrt_model_connect/benchmark/types.py",
        "python/tensorrt_model_connect/benchmark/worker.py",
    }
    assert python_files - benchmark_files == expected_python
    assert benchmark_files == expected_benchmark
    assert files("src") == expected_native
    assert files("include/trtmc") == expected_headers
    assert files("tools") == expected_tools
    assert files("cmake") == expected_cmake
    assert files("third_party") == expected_third_party
    assert files("scripts") == {"scripts/trtmc-bench"}
...
def test_applications_depend_only_on_public_model_connect_surfaces() -> None:
    ...
    application_roots = (
        REPO / "examples",
        REPO / "benchmarks",
        REPO / "python/tensorrt_model_connect/benchmark",
    )
    application_files = [
        path for root in application_roots for path in root.rglob("*")
    ] + [REPO / "scripts/trtmc-bench", REPO / "tools/perf_matrix.py"]
    for path in application_files:
        ...
        if path.suffix in {".cpp", ".h", ".hpp", ".cu"}:
            for include in re.findall(r'#include\s+[<"]([^>"]+)', source):
                if include.startswith(("src/", "families/")):
    ...
    for root in (REPO / "include", REPO / "src", REPO / "families"):
        for path in root.rglob("*"):
            ...
            if path.suffix in {".cpp", ".h", ".hpp", ".cu"}:
                for include in re.findall(r'#include\s+[<"]([^>"]+)', source):
                    if include.startswith(("examples/", "benchmarks/")):
                        violations.append(
                            f"{path.relative_to(REPO)}:reverse-include:{include}"
                        )
    for path in (REPO / "python/tensorrt_model_connect").rglob("*.py"):
        if "benchmark" in path.relative_to(REPO / "python/tensorrt_model_connect").parts:
            continue
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        for node in ast.walk(tree):
            modules: list[str] = []
            if isinstance(node, ast.Import):
                ...
                modules.append(node.module)
            for module in modules:
                if module == "tensorrt_model_connect.benchmark" or module.startswith(
                    "tensorrt_model_connect.benchmark."
                ):
                    violations.append(
...
def test_every_builder_handles_every_family_owned_request_field() -> None:
    build_api = REPO / "python/tensorrt_model_connect/build.py"
        ...
        for path in _reachable_family_python(family):
            ...
            if has_bin:
                ...
                if "ml_dtypes" not in imports or 'framework="numpy"' not in source:
                    violations.append(f"{path.relative_to(REPO)}:numpy-safetensors-contract")
    assert violations == []
...
def test_native_loader_and_preprocessing_stay_out_of_shared_core() -> None:
    ...
    assert "src/runtime/loader/family_loader.cpp" in runtime_sources
    ...
    for family in family_dirs():
        ...
        assert "trtmc_runtime" not in family_cmake

    assert not (REPO / "include/trtmc/trtmc_io.hpp").exists()
    assert (REPO / "src/cli/io.h").is_file()
    cli_io = (REPO / "src/cli/io.cpp").read_text(encoding="utf-8")
...
def test_every_runtime_exports_only_the_task_factory_contract() -> None:
    ...
    for family in family_dirs():
        ...
        if "${PROJECT_SOURCE_DIR}/src" in cmake:
...
def test_family_factory_receives_only_the_file_backed_reader_and_backend() -> None:
    factory_header = (REPO / "include/trtmc/runtime/family_factory.h").read_text(encoding="utf-8")
    ...
    loader = (REPO / "src/runtime/loader/family_loader.cpp").read_text(encoding="utf-8")
...
def test_every_manifest_task_has_a_concrete_family_implementation() -> None:
    violations: list[str] = []
    task_header = (REPO / "include/trtmc/task.h").read_text(encoding="utf-8")
    ...
    for family in family_dirs():
        ...
        for path in (family / "tests/manifests").glob("*.json"):
            ...
            for index, case in enumerate(manifest.get("testcases", [])):
                ...
                for field in sorted(case.keys() - consumed):
