...
from benchmarks.performance.baselines import lance_reference, sana_wm_reference, task_reference


REPO = Path(__file__).resolve().parents[4]
SUITE = REPO / "benchmarks/performance/release.yaml"
...
def test_reference_runner_dependencies_are_baseline_owned() -> None:
    root = REPO / "benchmarks/performance/baselines"
    required = {
        "audio_reference.py",
        "elf_reference.py",
        "lance_reference.py",
...
def test_timeseries_entries_use_current_forecast_request_schema(tmp_path: Path) -> None:
    ...
    source = (
        REPO / "benchmarks/performance/baselines/task_reference.py"
...
def test_qwen3_omni_preserves_thinker_and_talker_limits(tmp_path: Path) -> None:
    ...
    worker = (REPO / "examples/trtmc_benchmark_worker.cpp").read_text(encoding="utf-8")
